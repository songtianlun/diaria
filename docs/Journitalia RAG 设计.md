...
新架构的核心是**解耦 AI 模型服务与 Journitalia 应用本身**。Journitalia 不再捆绑任何特定的 AI 服务，而是通过用户提供的配置（API Endpoint, 模型名称, API Key）来与外部或本地的 AI 服务进行交互。

### 3.1. 推荐技术栈

| 组件 | 技术选型 | 关键优势 |
| :--- | :--- | :--- |
| **向量数据库** | **chromem-go** | **嵌入式、Go 原生、零依赖**。与 Journitalia 架构完美契合，无需额外服务进程，是实现简洁部署的关键。 [1] |
...
- **索引流程**: 当用户保存日记时，Journitalia 的 AI 服务层会读取用户配置，调用指定的 Embedding API 将日记文本向量化，并将结果存入 `chromem-go` 数据库。
...
在 Journitalia 的设置中新增"AI 助手"模块，主要包含：
...
- **安全**: 为了保护用户隐私，所有 `API Key` 在存入数据库前都将**使用 AES-256-GCM 进行加密**。密钥由 Journitalia 应用管理，确保数据安全。
...
- **简化部署**: 由于移除了 Ollama 的捆绑，Journitalia 的 Docker 部署将回归到单容器模式，降低了对用户服务器的资源要求。
...
同时，**精细化的对话历史管理功能**的设计，使得 AI 助手不再是“一次性”的工具，而是能够沉淀知识、提供连续性思考支持的智能伙伴，进一步增强了 Journitalia 的核心价值。
